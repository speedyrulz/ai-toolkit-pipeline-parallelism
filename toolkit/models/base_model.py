...
class BaseModel:
    ...
    def component_load_kwargs(self, role: str = "transformer", dtype=None):
        ...
        if mc.low_vram and role in ("transformer", "te"):
            ...
            device = self.vae_device_torch
        # pipeline sharding (transformer only): split the repeated blocks over
        # several gpus instead of placing the whole module on one. The primary
        # (first) device must be the trainer's device — the model's output
        # lands there and the loss math runs against tensors on it.
        pipeline_devices = None
        pipeline_balance = None
        if role == "transformer" and getattr(mc, "pipeline_sharding", False):
            ...
            pipeline_devices = mc.pipeline_devices
            if pipeline_devices is None:
                pipeline_devices = [primary] + [
                    f"cuda:{i}"
                    for i in range(torch.cuda.device_count())
                    if i != _train_idx
                ]
            else:
                pipeline_devices = [str(torch.device(d)) for d in pipeline_devices]
                if pipeline_devices[0] != primary:
                    if primary in pipeline_devices:
                        pipeline_devices.remove(primary)
                    pipeline_devices.insert(0, primary)
                    self.print_and_status_update(
                        f"pipeline_devices reordered to {pipeline_devices}: the "
                        f"trainer device ({primary}) must be the primary shard device"
                    )
            pipeline_balance = mc.pipeline_balance
            # the shard attach does the placement; a whole-module move to one

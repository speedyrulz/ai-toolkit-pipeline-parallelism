    ...
    def aitk_post_load(
        ...
        **_load_only_kwargs,  # tolerate component_load_kwargs() extras
        ...
        if pipeline_devices:
            ...
            status_fn(f"Sharding across gpus: {[str(d) for d in pipeline_devices]}")
            # block names live on the module for v2 models and on the holder
            # (base_model) for the older arch classes
            block_names = self.get_transformer_block_names()
            if block_names is None and base_model is not None:
                block_names = base_model.get_transformer_block_names()
            PipelineShardManager.attach(
                self,
                devices=pipeline_devices,
                block_names=block_names,
                # quantized weights only move devices; a dtype cast would
                # dequantize them
                dtype=None if getattr(self, "aitk_is_quantized", False) else dtype,
